...
class Hunter2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self):
        ...
        * /kobuki/laser/scan: Laser Readings
        ...
        super(Hunter2Env, self).__init__(controllers_list=self.controllers_list,
        ...
        self._cmd_vel_pub = rospy.Publisher('/ackermann_steering_controller/cmd_vel', Twist, queue_size=1)
    ...
    def _check_laser_scan_ready(self):
        ...
        while self.laser_scan is None and not rospy.is_shutdown():
            try:
                self.laser_scan = rospy.wait_for_message("/camera/scan", LaserScan, timeout=5.0)
                rospy.logdebug("Current /camera/scan READY=>")

            except:
                rospy.logerr("Current /camera/scan not ready yet, retrying for getting laser_scan")
                ...
                if item == float ('Inf') or numpy.isinf(item):
                   ...
                   pass
                    ...
                    if (item < min_laser_distance):
                        rospy.logerr("Hunter2 HAS CRASHED >>> item=" + str(item)+"< "+str(min_laser_distance))
